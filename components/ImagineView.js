      ...
      <div className="sample-scroll">
        {SAMPLE_PROMPTS.map((sample, i) => (
          <div key={i} className="sample-card">
            <img
              src={`/api/proxy-image?url=${encodeURIComponent(`https://image.pollinations.ai/prompt/${encodeURIComponent(sample.prompt)}?width=512&height=512&seed=${i * 7 + 42}&nologo=true`)}`}
              alt={sample.label}
...
export default function ImagineView({ isDark }) {
  ...
  return (
    <div className="imagine-page" style={{ flex: 1, overflowY: 'auto', padding: '1.5rem 1rem' }}>
      ...
      <div className="imagine-workspace">
        ...
        <button className="generate-btn" onClick={generate} disabled={loading || !prompt.trim()}>
          <span>✦</span> {loading ? 'Generating...' : 'Imagine'}
        </button>
